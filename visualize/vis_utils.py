from Common.system.smpl_animation_file import SMPLAnimationFile
...
class npy2obj:
    def __init__(self, motions, sample_idx, rep_idx, device=0, cuda=True):
        self.motions = motions
        self.rot2xyz = Rotation2xyz(device='cpu')
        self.faces = self.rot2xyz.smpl_model.faces
        self.njoints, self.nfeats, self.nframes = self.motions['motion'].shape
        self.opt_cache = {}
        ...
        self.num_frames = self.motions['motion'].shape[-1]
        ...
        if self.nfeats == 3:
            ...
            motion_tensor, opt_dict = self.j2s.joint2smpl(self.motions['motion'].transpose(2, 0, 1))  # [nframes, njoints, 3]
            self.motions['motion'] = motion_tensor.cpu().numpy()
        # elif self.nfeats == 6:
        #     self.motions['motion'] = self.motions['motion'][[self.absl_idx]]
        _, self.njoints, self.nfeats, self.nframes = self.motions['motion'].shape
        self.real_num_frames = self.motions['lengths']
    ...
    def save_npy(self):
        ...
        for idx, x in np.ndenumerate(self.motions['motion'][0, -1, :3, i]):   
            trans_raw.append(x)
        # smpl_trans = np.array(trans_raw).reshape(self.real_num_frames, 3)
        motion_file = SMPLAnimationFile().set_data({'smpl_poses': np.array(poses_raw).reshape(self.real_num_frames, 72), 'smpl_trans': np.array(trans_raw).reshape(self.real_num_frames, 3)})
        return motion_file
