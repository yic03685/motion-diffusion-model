...
def generate(promgram_args={}):
    args = generate_args(promgram_args)
    fixseed(args.seed)
    ...
    # Now only support one sample per generation for api
    motion_file = MotionFile().set_data(
        {
    ...
    if hml_sample is not None:
        motion_hml_file = MotionFile().set_data(
            {
                "motion": hml_sample,
                "text": all_text[0],
                "lengths": all_lengths[0],
    ...
    # The following save additional animation files
    if promgram_args["debug"] is True:
    ...
    return {"smpl_file": motion_file, "hml_file": motion_hml_file}
